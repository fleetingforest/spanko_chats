...
def _get_file_content_from_storage(storage_path):
    ...
    try:
        ...
        try:
            ...
            try:
                file_content = file_content_bytes.decode('latin-1') # Try latin-1 as a common fallback
            except UnicodeDecodeError:
                print(f"Warning: Could not decode file {storage_path} as UTF-8 or Latin-1. Treating as binary/non-text.")
...
@app.route('/upload_file_to_chat', methods=['POST'])
def upload_file_to_chat():
    ...
    if file and allowed_file(file.filename):
        ...
        chat_id = session.get('active_chat_id', 'unknown_chat') # Ensure active_chat_id is in session

        if chat_id == 'unknown_chat':
            # Fallback or error if chat_id is not set,
            # or create a temporary chat_id if that's part of the design for new chats.
            # For now, let's assume active_chat_id should be present.
            # If not, a new chat should be initiated first, or this endpoint needs a chat_id.
            ...
            if not chat_id:
                # This case should ideally be handled by ensuring a chat session is active.
                # If a file is uploaded before a chat is formally started,
                # it might be associated with a temporary ID or a default "pre-chat" bucket.
                # For now, let's use a generic placeholder if no chat_id exists.
                # This implies the file is uploaded before the first message that creates the chat.
                # This might require adjustment based on when active_chat_id is set.
                # A robust solution would be to pass chat_id from client if it exists,
                # or have client ensure chat is initiated.
                # For now, let's assume active_chat_id is usually set.
                # If not, this is a potential issue.
                # Let's default to a temporary chat_id if not found, but this is not ideal.
                chat_id = f"temp_chat_for_{user_email.split('@')[0]}"
                print(f"Warning: active_chat_id not found in session for file upload. Using temporary: {chat_id}")


        unique_id = uuid.uuid4().hex
        storage_filename = f"{unique_id}_{filename}"
        storage_path = f"user_uploads/{user_email}/{chat_id}/{storage_filename}"

        try:
            blob = bucket.blob(storage_path)
            blob.upload_from_file(file.stream, content_type=file.content_type)
            file_size = file.tell() # Get file size after stream has been read by upload_from_file

            file_info = {
                'name': filename,
                'storage_path': storage_path,
                'type': file.content_type,
                'size': file_size # file.content_length might be -1 if client doesn't send it
            }
            
            # DO NOT add to chat history here. Return file_info to client.
            # The client will send this file_info with the next message.
            print(f"File {filename} uploaded to {storage_path}. Info: {file_info}")
            return jsonify({'message': 'File uploaded successfully, awaiting message.', 'file_info': file_info}), 200
...
def chat():
    ...
    messages_for_ai = _prepare_messages_for_ai(system_prompt, active_conversation_obj.get('messages', []))
